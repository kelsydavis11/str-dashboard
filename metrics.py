...
def active(df: pd.DataFrame) -> pd.DataFrame:
    """Usable, non-cancelled bookings — the base for realised/forecast metrics."""
    if df.empty:
        return df
...
def overlapping(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    """Active stays whose [check_in, check_out) intersects the window [start, end)."""
    a = active(df)
    if a.empty:
        return a
...
def allocate(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    """Add nights_in / net_in / gross_in: the portion of each stay inside the window."""
...
def period_summary(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp,
                   units: int) -> dict:
    """Occupancy, RevPAR, ADR and night-allocated revenue over [start, end)."""
...
def period_by_dimension(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp,
                        dim: str) -> pd.DataFrame:
    """Night-allocated net/gross/nights per property or platform for the window."""
    a = allocate(df, start, end)
    if a.empty:
        return pd.DataFrame()
    g = a.groupby(dim).agg(
...
def monthly_series(df: pd.DataFrame, units: int, start: pd.Timestamp,
                   end: pd.Timestamp) -> pd.DataFrame:
    """Per-calendar-month occupancy / RevPAR / net revenue across [start, end)."""
...
def pace_by_checkin_month(df: pd.DataFrame, today: pd.Timestamp) -> pd.DataFrame:
    """Booking pace: net payout by CHECK-IN month (booking-level), past vs future."""
    ...
    agg["period"] = agg["month"].apply(
        lambda m: "Future" if m.to_timestamp() >= cutoff else "Past")
    agg["month"] = agg["month"].dt.strftime("%b %Y")
    return agg


def cancellation_stats(df: pd.DataFrame) -> dict:
    """Cancellation reporting — the one place cancelled rows are included."""
    total = len(df)
