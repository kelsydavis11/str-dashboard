...
<style>
/* ---- KPI strip ---- */
.kstrip { display:grid; grid-template-columns:repeat(6,1fr); background:var(--surface);
  border:1px solid var(--rule); border-radius:4px; overflow:hidden; margin:0 0 30px; }
...
.cal-tools a { height:28px; box-sizing:border-box; display:inline-flex; align-items:center; text-decoration:none;
  color:var(--ink); border:1px solid var(--rule); background:var(--surface); border-radius:3px; font-weight:400; font-size:12px; }
...
.pill { pointer-events:auto; text-decoration:none !important; display:flex; align-items:center; height:20px; margin:0 3px;
  padding:0 8px; border-radius:3px; font-size:11.5px; font-weight:400; white-space:nowrap; overflow:hidden; transition:filter .12s; }
...
/* ---- operational agenda (next 7 days) ---- */
...
def _pill_html(seg, ym, suffix, colors) -> str:
    ...
    if seg["current"] and not b["cancelled"]:
        cls.append("current")
    if seg["cont_left"]:
        cls.append("cl")
    if seg["cont_right"]:
...
def _panel_html(prop_key, bookings, weeks, today, ym, suffix, occupied, turns, kpi) -> str:
    ...
    parts = [f'<div class="cal-panel"><div class="cal-phead">'
             f'<div class="cal-pname" style="color:{colors["base"]}">'
             f'<span class="cal-dot" style="background:{colors["base"]}"></span>'
             f'{html.escape(colors["short"])}</div>'
             f'<div class="cal-pkpi">{kpi}</div></div>']
    ...
    for week in weeks:
        ...
        for d in week:
            ...
            if d == today:
                ...
                classes.append("we")
            diamond = ('<span class="diamond" title="Same-day turnover"></span>'
                       if d in turns and in_month else "")
            cells.append(f'<div class="{" ".join(classes)}"{title} style="min-height:{cell_h}px">'
                         f'<div class="cal-cellhead"><span class="cal-daynum">{d.day}</span>'
                         f'{diamond}</div></div>')
        bars = "".join(_pill_html(s, ym, suffix, colors) for s in segs)
        parts.append(f'<div class="cal-week"><div class="cal-bg">{"".join(cells)}</div>'
                     f'<div class="cal-bars">{bars}</div></div>')
    parts.append("</div>")
    return "".join(parts)
...
def _detail_card(df, booking_id, ym, suffix):
    ...
    fields = "".join([
        f("Guest", html.escape(str(r.get("guest") or "—"))),
        f("Property", html.escape(str(r["property"]))),
        f("Platform", f'<span class="cal-tag">{html.escape(str(r["platform"]))}</span>'),
        f("Booking ID", html.escape(str(r["booking_id"]))),
        f("Check-in", d(r["check_in"])), f("Check-out", d(r["check_out"])),
        f("Nights", nights), f("Guests", guests),
        f("Booked on", d(r["booked_on"])), f("Lead time", lead),
        f("Net payout", _money(r["revenue_net"]) or "—"),
        f("Nightly rate", _money(nightly) or "—"),
        f("Status", html.escape(str(r["status"]))),
    ])
    st.markdown(
        f'<div class="cal-detail"><div class="dh"><span class="dt">'
        f'{html.escape(str(r.get("guest") or "Reservation"))}</span>'
        f'<a class="x" href="?ym={ym}{suffix}" target="_self">✕ close</a></div>'
        f'<div class="grid">{fields}</div></div>', unsafe_allow_html=True)
...
def _agenda(df, today, ym, suffix):
    """Operational agenda: who's in-house now, then arrivals / departures /
    turnovers grouped by day for the next 7 days, with Today / Tomorrow first."""
    props = THEME["props"]

    def prop(pk):
        return props.get(pk, {"base": "#787E88", "short": str(pk)})

    a = df[df["property_raw"].isin(PROPERTIES) & (~df["is_cancelled"]) & df["usable"]].copy()
    ci = a["check_in"].dt.normalize()
    co = a["check_out"].dt.normalize()

    # in-house now
    now = a[(ci <= today) & (co > today)].sort_values("check_out")
    now_items = []
    for _, r in now.iterrows():
        c = prop(r["property_raw"])
        d = r["check_out"].normalize()
        out = "today" if d == today else ("tomorrow" if d == today + DAY else f"{d:%a} {d.day}")
        now_items.append(
            f'<span class="now-item"><span class="dot" style="background:{c["base"]}"></span>'
            f'{html.escape(str(r.get("guest") or "Guest"))} · {html.escape(c["short"])} · out {out}</span>')
    now_html = ('<div class="ag-now"><span class="lbl">In-house now</span>'
                + ("".join(now_items) if now_items
                   else '<span class="now-empty">Both properties vacant</span>') + "</div>")
    ...
    def ev(typ, tag, body, bid, meta=""):
        ...
        return (f'<a class="ag-ev" href="{href}" target="_self"><span class="ev-tag {typ}">{tag}</span>'
                f'<span class="ev-body">{body}</span>{meta_html}</a>')
    ...
    for i in range(7):
        ...
        for pk in PROPERTIES:
            c = prop(pk)
            dep = deps[deps["property_raw"] == pk]
            arr = arrs[arrs["property_raw"] == pk]
            if len(dep) and len(arr):
                dg, ag = dep.iloc[0], arr.iloc[0]
                n = _nights(ag)
                evs.append(ev("turnover", "Turnover",
                              f'<b>{html.escape(c["short"])}</b> — {html.escape(str(dg.get("guest") or ""))}'
                              f' out → {html.escape(str(ag.get("guest") or ""))} in', ag["booking_id"],
                              f'{n}-night stay' if n else ""))
            elif len(dep):
                dg = dep.iloc[0]
                evs.append(ev("checkout", "Check-out",
                              f'{html.escape(str(dg.get("guest") or ""))} · {html.escape(c["short"])}',
                              dg["booking_id"]))
            elif len(arr):
                ag = arr.iloc[0]
                n = _nights(ag)
                evs.append(ev("checkin", "Check-in",
                              f'{html.escape(str(ag.get("guest") or ""))} · {html.escape(c["short"])}',
                              ag["booking_id"], f'{n}-night stay' if n else ""))
        event_count += len(evs)
        if not evs and i > 1:
            continue  # skip quiet days beyond tomorrow
        ...
        day_blocks.append(
            f'<div class="ag-day{" is-today" if i == 0 else ""}">'
            f'<div class="ag-daylabel">{rel_html}<span class="abs">{D:%a} {D.day} {D:%b}</span></div>'
            f'<div class="ag-rows">{rows}</div></div>')

    st.markdown(
        f'<div class="ag"><div class="ag-head"><span class="t">Next 7 days</span>'
        f'<span class="c">{event_count} arrival{"" if event_count == 1 else "s"} & departure'
        f'{"" if event_count == 1 else "s"}</span></div>'
        f'{now_html}{"".join(day_blocks)}</div>', unsafe_allow_html=True)


def render(df, today, period_start=None):
    st.markdown(CAL_CSS, unsafe_allow_html=True)

    # The calendar month follows the sidebar reporting period (its start month);
    # the ‹ › / Today links then step within it via the ym query param. When the
    # sidebar period changes we override ym so the view follows the filter.
    period_month = (pd.Timestamp(period_start).replace(day=1).normalize()
                    if period_start is not None else today.replace(day=1))
    ...
    if st.session_state.get("_cal_period") != period_ym:
        ...
        st.query_params["ym"] = period_ym          # sidebar changed → snap to it
        month_start = period_month
    else:
        q = st.query_params.get("ym")
        try:
            month_start = pd.Timestamp((q or period_ym) + "-01").normalize()
        except Exception:
            month_start = period_month
    ...
    per = {}
    booked_total = vac_total = turn_total = vac_weekend = changeover_total = 0
    for pk in PROPERTIES:
        ...
        booked_total += len(occ)
        vac_total += len(vac)
        turn_total += len(turns)
        changeover_total += chg
    ...
    # prior-month comparison + weekday/weekend ADR (footnote only)
    ...
    for pk in PROPERTIES:
        for b in per[pk]["bk"]:
            if b["cancelled"] or pd.isna(b["net"]):
                continue
            ...
            while d <= b["last_night"]:
                if d.month == target_month:
                    if d.weekday() >= 4:
                        wk_rev += nightly; wk_n += 1
                    else:
                        wd_rev += nightly; wd_n += 1
                d += DAY
    wknd_adr = wk_rev / wk_n if wk_n else float("nan")
    wkdy_adr = wd_rev / wd_n if wd_n else float("nan")
    vac_mid = vac_total - vac_weekend

    net_foot = (f"vs {_money_k(net_prev)} in {prev_start:%B}" if net_prev > 0
                else f"no {prev_start:%B} data")
    occ_cls = "pos" if pts >= 0 else "neg"
    occ_foot = f'{abs(pts):.0f} pts {"above" if pts >= 0 else "below"} {TARGET_OCC*100:.0f}% target'

    strip = "".join([
        f'<div class="kc"><div class="kl">Occupancy</div>'
        f'<div class="krow"><span class="kv">{occ_pct*100:.0f}%</span></div>'
        f'<div class="track"><i style="width:{min(occ_pct*100,100):.0f}%"></i>'
        f'<b style="left:{TARGET_OCC*100:.0f}%"></b></div>'
        f'<div class="kf {occ_cls}">{occ_foot}</div></div>',

        f'<div class="kc"><div class="kl">Booked nights</div>'
        f'<div class="krow"><span class="kv">{booked_total}<span class="u"> / {available}</span></span></div>'
        f'<div class="kf">{vac_total} night{"" if vac_total == 1 else "s"} unsold</div></div>',

        f'<div class="kc"><div class="kl">Net revenue</div>'
        f'<div class="krow"><span class="kv">{_money_k(combined["net_revenue"])}</span>'
        f'{_delta(combined["net_revenue"], net_prev)}</div>'
        f'<div class="kf">{net_foot}</div></div>',

        f'<div class="kc"><div class="kl">ADR</div>'
        f'<div class="krow"><span class="kv">{_money(combined["adr_net"]) or "—"}</span>'
        f'{_delta(combined["adr_net"], adr_prev)}</div>'
        f'<div class="kf">Wkdy {_money(wkdy_adr) or "—"} · Wknd {_money(wknd_adr) or "—"}</div></div>',

        f'<div class="kc"><div class="kl">Vacant nights</div>'
        f'<div class="krow"><span class="kv warn">{vac_total}</span></div>'
        f'<div class="kf">{vac_weekend} on Fri–Sat · {vac_mid} midweek</div></div>',

        f'<div class="kc"><div class="kl">Turnovers</div>'
        f'<div class="krow"><span class="kv">{turn_total}</span></div>'
        f'<div class="kf">of {changeover_total} changeover days</div></div>',
    ])
    ...
    today_txt = (f'Today is {today:%A} {today.day} {today:%B}'
                 if month_start.month == today.month and month_start.year == today.year else "")
    st.markdown(
        f'<div class="cal-tb"><div class="cal-tbl">'
        f'<span class="cal-month">{month_start:%B %Y}</span>'
        f'<span class="cal-todaytxt">{today_txt}</span></div>'
        f'<div class="cal-tbr"><div class="cal-legend">'
        f'<span><i class="lg-book"></i> Booked</span>'
        f'<span><i class="lg-vac"></i> Vacant</span>'
        f'<span><i class="lg-turn"></i> Turnover</span></div>'
        f'<div class="cal-tools">'
        f'<a class="ico" href="?ym={prev_ym}{suffix}" target="_self">‹</a>'
        f'<a class="today" href="?ym={cur_ym}{suffix}" target="_self">Today</a>'
        f'<a class="ico" href="?ym={next_ym}{suffix}" target="_self">›</a>'
        f'<a class="{toggle_cls}" href="{toggle_href}" target="_self">{toggle_lbl}</a>'
    ...
    for pk in PROPERTIES:
        ...
        kpi = (f'<span><span class="pl">Occ</span><b>{s["occupancy"]*100:.0f}%</b></span>'
               f'<span><b>{netk}</b></span>'
               f'<span><span class="pl">ADR</span><b>{_money(s["adr_net"]) or "—"}</b></span>')
        panels.append(_panel_html(pk, per[pk]["bk"], weeks, today, ym, suffix,
                                  per[pk]["occ"], per[pk]["turns"], kpi))
    ...
    for col, pk in zip(fcols, PROPERTIES):
        p = per[pk]
        dates = ", ".join(str(d.day) for d in p["vac"])
        col.markdown(
            f'<div class="cal-foot"><span><b>{len(p["vac"])} vacant nights</b>'
            f'{(" · " + dates) if dates else ""}</span>'
            f'<span>{len(p["turns"])} turnovers</span></div>', unsafe_allow_html=True)
