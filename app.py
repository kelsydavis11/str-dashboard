...
def bar(df, x, y, color=None, y_title="", fmt="$,.0f", scheme=None):
    ...
    if color:
        rng = scheme or (list(theme.PLATFORM_COLORS.values())
                         if color == "platform" else list(theme.PROPERTY_COLORS.values()))
        enc["color"] = alt.Color(f"{color}:N", legend=None, scale=alt.Scale(range=rng))
    return alt.Chart(df).mark_bar(cornerRadiusTopLeft=4, cornerRadiusTopRight=4,
                                  size=46).encode(**enc).properties(height=280)


def month_bar(df, y, y_title, fmt, color=theme.ACCENT):
    return alt.Chart(df).mark_bar(size=32, cornerRadiusTopLeft=4,
                                  cornerRadiusTopRight=4, color=color).encode(
...
try:
    ...
    if any(t in msg for t in ("invalid_grant", "expired or revoked", "refresh")):
        ...
        st.markdown(
            "Re-authorise (opens a Google consent screen), then restart the app:\n\n"
...
# Data is cached for 10 min; this pulls a fresh copy from the sheet on demand.
...
# Reporting period — a real calendar window, live from today.
month_start = TODAY.replace(day=1)
act_all = m.active(df)
data_start = act_all["check_in"].min().normalize() if not act_all.empty else TODAY
data_end = (act_all["check_out"].max().normalize() + pd.Timedelta(days=1)
            if not act_all.empty else TODAY + pd.Timedelta(days=1))
PRESETS = {
...
if choice == "Custom…":
    c = sb.date_input("Custom range", value=(month_start.date(),
                      (month_start + pd.DateOffset(months=1)).date()))
...
sb.caption(f"Window: {p_start:%d %b %Y} → {(p_end - pd.Timedelta(days=1)):%d %b %Y}. "
           "Occupancy counts any stay overlapping it.")

# dimension-filtered set (NOT time-filtered — the period drives occupancy itself)
...
# ------------------------------------------------------------------- header
st.markdown(
    '<div class="hdr-bar"><div class="hdr-left">'
    '<div class="eyebrow">Portfolio</div>'
    '<div class="pg-title">Short-term rental performance</div></div>'
    '<div class="hdr-btns"><span class="pg-btn">Export</span></div></div>',
    unsafe_allow_html=True)

CONTEXT_LINE = (f'<div class="pg-context">Confirmed bookings · {dq.rows_loaded} stays · '
                f'{choice} · as at {TODAY:%d %b %Y}</div>')

tabs = st.tabs(["Calendar", "Overview", "Revenue & pace", "Operations",
                "Properties", "Cancellations"])

# ----------------------------------------------------------------- CALENDAR
with tabs[0]:
    # Calendar shows both properties; its month follows the sidebar reporting
    # period (the ‹ › arrows step within it). p_start drives the displayed month.
...
with tabs[1]:
    cards([
        ...
        ("Occupancy", pct(ps["occupancy"]),
         f'{num(ps["occupied_nights"])}/{num(ps["available_nights"])} nights'),
    ...
    with c1:
        ...
        if not pr.empty:
            st.altair_chart(bar(pr, "property", "net_revenue", "property"),
    ...
    with c2:
        ...
        if not pl.empty:
            st.altair_chart(bar(pl, "platform", "net_revenue", "platform"),
                            use_container_width=True)

    st.markdown('<div class="section-h">Portfolio by stay timing (all dates)</div>',
                unsafe_allow_html=True)
    timing = (m.active(fdf).groupby("stay_state")["revenue_net"].sum()
              .reindex(["Historical", "Current", "Future"]).fillna(0).reset_index())
    st.altair_chart(bar(timing, "stay_state", "revenue_net", "stay_state",
                        scheme=["#b9c0c7", "#c98a3a", "#2f6f6a"]), use_container_width=True)

# ------------------------------------------------------------ REVENUE & PACE
with tabs[2]:
    st.caption("Forward view — independent of the reporting period above.")
    st.markdown('<div class="section-h">Booking pace — net payout by check-in month</div>',
                unsafe_allow_html=True)
    pace = m.pace_by_checkin_month(fdf, TODAY)
    if not pace.empty:
        chart = alt.Chart(pace).mark_bar(size=34, cornerRadiusTopLeft=4,
                                         cornerRadiusTopRight=4).encode(
            x=alt.X("month:N", sort=list(pace["month"]), title=None,
                    axis=alt.Axis(labelAngle=-40)),
            y=alt.Y("revenue_net:Q", title="Net payout", axis=alt.Axis(format="$,.0f")),
            color=alt.Color("period:N", scale=alt.Scale(
                domain=["Past", "Future"], range=["#b9c0c7", "#2f6f6a"]),
                legend=alt.Legend(title=None, orient="top")),
            tooltip=["month", alt.Tooltip("revenue_net:Q", format="$,.0f"), "period"],
        ).properties(height=300)
        st.altair_chart(chart, use_container_width=True)

    st.markdown('<div class="section-h">Monthly occupancy — next 12 months</div>',
                unsafe_allow_html=True)
    mo = m.monthly_series(fdf, units, month_start, month_start + pd.DateOffset(months=12))
    if not mo.empty:
        st.altair_chart(month_bar(mo, "occupancy", "Occupancy", ".0%"),
                        use_container_width=True)

# ----------------------------------------------------------------- OPERATIONS
with tabs[3]:
    st.caption(f"Activity within {p_start:%d %b} → {(p_end - pd.Timedelta(days=1)):%d %b %Y}.")
    a = m.allocate(fdf, p_start, p_end)  # stays overlapping the window
    arr = m.active(fdf)
    arrivals = int(((arr["check_in"] >= p_start) & (arr["check_in"] < p_end)).sum())
    departures = int(((arr["check_out"] >= p_start) & (arr["check_out"] < p_end)).sum())
    cards([
        ("Stays in period", num(len(a)), "overlapping the window"),
        ("Arrivals in period", num(arrivals), ""),
        ("Departures in period", num(departures), ""),
    ])
    st.write("")
    st.markdown('<div class="section-h">Stays overlapping the period</div>', unsafe_allow_html=True)
    if not a.empty:
        view = a.sort_values("check_in")[
            ["property", "platform", "guest", "check_in", "check_out",
             "los", "nights_in", "revenue_net", "stay_state"]].copy()
        for dcol in ("check_in", "check_out"):
            view[dcol] = view[dcol].apply(day)
        view["revenue_net"] = view["revenue_net"].apply(money)
        view = view.rename(columns={
            "property": "Property", "platform": "Platform", "guest": "Guest",
            "check_in": "Check-in", "check_out": "Check-out", "los": "Nights",
            "nights_in": "Nights in period", "revenue_net": "Net payout",
            "stay_state": "Timing"})
...
with tabs[4]:
    ...
    for p in sel_props:
        ...
        rows.append({
            "Property": p, "Occupancy": pct(s["occupancy"]),
            "Net payout": money(s["net_revenue"]), "Nights": num(s["occupied_nights"]),
            "ADR (net)": money(s["adr_net"]), "RevPAR": money(s["revpar"]),
            "Stays": num(s["bookings"]),
        })
    if rows:
        ...
        if not pr.empty:
            st.altair_chart(bar(pr, "property", "net_revenue", "property",
                                y_title="Net payout (period)"), use_container_width=True)

# ---------------------------------------------------------------- CANCELLATIONS
with tabs[5]:
    st.caption("All cancellations for the selected properties/platforms (all dates).")
    cstats = m.cancellation_stats(dim)  # ignores the status filter by design
    cards([
        ("Cancellation rate", pct(cstats["rate"]), f'{cstats["cancelled"]} of {cstats["total"]}'),
        ("Cancelled bookings", num(cstats["cancelled"]), ""),
        ("Lost net payout", money(cstats["lost_net"]), "value of cancellations"),
    ])
    st.write("")
    if cstats["by_platform"]:
        bp = pd.DataFrame([{"platform": k2, "cancelled": v}
                           for k2, v in cstats["by_platform"].items()])
        st.markdown('<div class="section-h">Cancellations by platform</div>', unsafe_allow_html=True)
        st.altair_chart(bar(bp, "platform", "cancelled", "platform",
                            y_title="Cancelled", fmt="d"), use_container_width=True)

# --------------------------------------------------------------- data-quality
with st.expander("Data quality & assumptions"):
    st.markdown(
        f"""
- **Rows loaded:** {dq.rows_loaded} · **usable:** {dq.rows_usable} ·
  **cancelled:** {dq.cancelled_rows} · **manual overrides used:** {dq.manual_overrides_used}
- **Occupancy model:** nights occupied within the reporting window ÷ nights available
  ({units} unit(s) × window nights). **Any stay overlapping the window counts** — check-in
  date is not used to include/exclude. Revenue is allocated night-by-night.
- **Cancelled and invalid-date rows are excluded** from occupancy/revenue everywhere
  (shown only on the Cancellations tab), per the reporting rules.
- **Occupancy assumes {units} always-available unit(s)** — owner-blocked/maintenance
  nights are not modelled (the `Calendar` tab could refine this later).
- **Net payout** parsed from text (`$1,430` → number); **Guest total** kept as gross.
  **Booking.com** rows carry no fee breakdown or guest headcount.
"""
    )
