...
# Must match the scope token.json was granted by the Node sync (full
# 'spreadsheets'). A narrower scope on refresh triggers invalid_scope. Read-only
# in practice — this app never issues a write call.
SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]

# Header text (lowercased, matched by substring) -> canonical column name.
COLUMN_MAP = {
    ...
    "guests": "guests",   # headcount (col "Guests")
    "guest": "guest",     # guest name (col "Guest") — resolved after "guests"
    "check-in": "check_in",
    "check-out": "check_out",
    "cancelled on": "cancelled_on",
    "nights": "nights",
    "lead time": "lead_time_days",
    "net payout (manual)": "net_payout_manual",
...
def _secrets():
    """Streamlit secrets when running inside Streamlit; None otherwise/on error."""
    try:
...
def _credentials():
    """Cloud: a service account or a pasted token in st.secrets.
    Local: the token.json from the Booking.com sync."""
    sec = _secrets()
    if sec is not None:
        try:  # 1) service account — recommended for a deployed/shared app
            ...
            pass
        try:  # 2) authorized-user token pasted into secrets (expires — see DEPLOY.md)
            if "google_token" in sec:
                return Credentials.from_authorized_user_info(dict(sec["google_token"]), SCOPES)
        except Exception:
            pass
    if TOKEN_FILE.exists():  # 3) local dev
        return Credentials.from_authorized_user_file(str(TOKEN_FILE), SCOPES)
    raise RuntimeError(
        "No Google credentials. Set st.secrets ('gcp_service_account' or 'google_token') "
        "for cloud, or create token.json locally.")
...
def _fetch_rows() -> list[list[str]]:
    ...
    resp = (
        ...
        .values()
        .get(spreadsheetId=_read_sheet_id(), range=f"'{SHEET_TAB}'!A1:AZ2000")
        .execute()
    )
...
def _resolve_columns(header: list[str]) -> dict[str, int]:
    """Map canonical names to column indices by header substring match."""
    norm = [re.sub(r"\s+", " ", (h or "")).strip().lower() for h in header]
    resolved: dict[str, int] = {}
    # Longer keys first so "net payout (manual)" wins over "net payout".
...
def load_bookings() -> tuple[pd.DataFrame, DataQuality]:
    ...
    # --- typed parsing (defensive: errors -> NaN/NaT) ---
    ...
    dq.unmapped_properties = sorted(
        {p for p, s in zip(df.get("property_raw", []), df["property"]) if s == p and p}
    )

    status = df.get("status", pd.Series("", index=df.index)).astype(str).str.strip()
    df["is_cancelled"] = status.str.lower().eq("cancelled") | df.get(
        "cancelled_on", pd.Series(pd.NaT, index=df.index)
    ).notna()
    dq.cancelled_rows = int(df["is_cancelled"].sum())

    # Preferred net revenue: manual override when present, else scraped net payout.
    ...
    # Length of stay: prefer Nights, else derive from dates.
    derived_los = (df["check_out"] - df["check_in"]).dt.days
    df["los"] = df.get("nights").fillna(derived_los)
    # Lead time: prefer column, else derive.
    ...
    df["usable"] = ~bad_dates  # revenue can be 0/NaN and still count for occupancy
    ...
    if dq.guests_blank:
        dq.messages.append(
            f"{dq.guests_blank} rows have no guest headcount (Booking.com does not "
            "supply it) — per-guest metrics would be Airbnb-only."
        )
    if dq.dropped_missing_dates:
        dq.messages.append(
            f"{dq.dropped_missing_dates} rows dropped from occupancy/ADR: missing or "
